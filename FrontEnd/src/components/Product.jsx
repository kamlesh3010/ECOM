...
const Product = () => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const response = await axios.get(`http://localhost:8061/api/product/${id}`);
    ...
    const fetchImage = async () => {
      try {
        const response = await axios.get(`http://localhost:8061/api/product/${id}/image`, {
          responseType: "blob",
        });
  ...
  const deleteProduct = async () => {
    try {
      await axios.delete(`http://localhost:8061/api/product/${id}`);
      removeFromCart(id);
